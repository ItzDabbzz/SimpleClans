...
public final class PermissionsManager {
    ...
    public PermissionsManager() {
        ...
        try {
            ...
            CookClans.log("[SimpleClans] Vault not found. No economy or extended Permissions support.");
        }
    }
    ...
    public void loadPermissions() {
        CookClans.getInstance().getSettingsManager().load();
        permissions.clear();
        for (Clan clan : plugin.getClanManager().getClans()) {
            permissions.put(clan.getTag(), CookClans.getInstance().getConfig().getStringList("permissions." + clan.getTag()));
        }
    }
    ...
    public void savePermissions() {
        for (Clan clan : plugin.getClanManager().getClans()) {
            if (permissions.containsKey(clan.getTag())) {
                CookClans.getInstance().getSettingsManager().getConfig().set("permissions." + clan.getTag(), getPermissions(clan));
            }
        }
        CookClans.getInstance().getSettingsManager().save();
    }
        ...
        if (cp != null && cp.toPlayer() != null) {
            ...
            if (permissions.containsKey(cp.getClan().getTag())) {
                if (!permAttaches.containsKey(cp.toPlayer())) {
                    permAttaches.put(cp.toPlayer(), cp.toPlayer().addAttachment( CookClans.getInstance()));
                }
