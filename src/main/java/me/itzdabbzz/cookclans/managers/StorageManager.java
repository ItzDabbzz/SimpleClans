package me.itzdabbzz.cookclans.managers;

import com.google.common.base.Charsets;
import me.itzdabbzz.cookclans.*;
import me.itzdabbzz.cookclans.storage.DBCore;
import me.itzdabbzz.cookclans.storage.MySQLCore;
import me.itzdabbzz.cookclans.storage.SQLiteCore;
import me.itzdabbzz.cookclans.uuid.UUIDFetcher;
...
public final class StorageManager {
    ...
    public void initiateDB() {
        if (plugin.getSettingsManager().isUseMysql()) {
            ...
            if (core.checkConnection()) {
                CookClans.log("[SimpleClans] " + plugin.getLang("mysql.connection.successful"));

                if (!core.existsTable("sc_clans")) {
                    CookClans.log("Creating table: sc_clans");
                ...
                if (!core.existsTable("sc_players")) {
                    CookClans.log("Creating table: sc_players");
                ...
                if (!core.existsTable("sc_kills")) {
                    CookClans.log("Creating table: sc_kills");
                ...
                CookClans.getInstance().getServer().getConsoleSender().sendMessage("[SimpleClans] " + ChatColor.RED + plugin.getLang("mysql.connection.failed"));
            }
        } else {
            core = new SQLiteCore(plugin.getDataFolder().getPath());

            if (core.checkConnection()) {
                CookClans.log("[SimpleClans] " + plugin.getLang("sqlite.connection.successful"));
                ...
                CookClans.getInstance().getServer().getConsoleSender().sendMessage("[SimpleClans] " + ChatColor.RED + plugin.getLang("sqlite.connection.failed"));
            }
        }
    }
    ...
    public void importFromDatabase() {
        ...
        if (!clans.isEmpty()) {
            CookClans.log(MessageFormat.format("[SimpleClans] " + plugin.getLang("clans"), clans.size()));
        }
        ...
        if (!cps.isEmpty()) {
            CookClans.log(MessageFormat.format("[SimpleClans] " + plugin.getLang("clan.players"), cps.size()));
        }
    }
    ...
    public void importFromDatabaseOnePlayer(Player player) {
        ...
        if (cp != null) {
            ...
            CookClans.log("[SimpleClans] ClanPlayer Reloaded: " + player.getName() + ", UUID: " + player.getUniqueId().toString());
        }
    }
    ...
    private void purgeClans(List<Clan> clans) {
        ...
        for (Clan clan : purge) {
            CookClans.log("[SimpleClans] " + MessageFormat.format(plugin.getLang("purging.clan"), clan.getName()));
            deleteClan(clan);
            clans.remove(clan);
        }
    }
    ...
    private void purgeClanPlayers(List<ClanPlayer> cps) {
        ...
        for (ClanPlayer cp : purge) {
            CookClans.log("[SimpleClans] " + MessageFormat.format(plugin.getLang("purging.player.data"), cp.getName()));
            deleteClanPlayer(cp);
            cps.remove(cp);
        }
    }
    ...
    public List<Clan> retrieveClans() {
        ...
        if (res != null) {
            try {
                ...
                CookClans.getLog().severe(String.format("An Error occurred: %s", ex.getErrorCode()));
                CookClans.getLog().log(Level.SEVERE, null, ex);
            }
        }
            ...
            try {
                while (res.next()) {
                    try {
                        ...
                        if (!tag.isEmpty()) {
                            Clan clan = CookClans.getInstance().getClanManager().getClan(tag);
                ...
                while (res.next()) {
                    try {
                        ...
                        if (!tag.isEmpty()) {
                            ...
                            if (clan != null) {
                                Clan clanReSync = CookClans.getInstance().getClanManager().getClan(tag);
                                ...
                                Clan newclan = CookClans.getInstance().getClanManager().getClan(clanDB.getTag());
                                cp.setClan(newclan);
                            }
                        }
            ...
            try {
                while (res.next()) {
                    try {
                        ...
                        CookClans.getLog().info(ex.getMessage());
    ...
    private void updatePlayersToUUID() {
        CookClans.log("[SimpleClans] Starting Migration to UUID Players !");
        CookClans.log("[SimpleClans] ==================== ATTENTION DONT STOP BUKKIT ! ==================== ");
        CookClans.log("[SimpleClans] ==================== ATTENTION DONT STOP BUKKIT ! ==================== ");
        CookClans.log("[SimpleClans] ==================== ATTENTION DONT STOP BUKKIT ! ==================== ");
        CookClans.getInstance().setUUID(false);
        List<ClanPlayer> cps = retrieveClanPlayers();
        ...
        for (ClanPlayer cp : cps) {
            try {
                UUID uuidPlayer;
                if ( CookClans.getInstance().getServer().getOnlineMode()) {
                ...
                core.update(query3);
                CookClans.log("[" + i + " / " + cps.size() + "] Success: " + cp.getName() + "; UUID: " + uuidPlayer.toString());
            } catch (Exception ex) {
                CookClans.log("[" + i + " / " + cps.size() + "] Failed [ERRO]: " + cp.getName() + "; UUID: ???");
            }
            i++;
        }
        CookClans.log("[SimpleClans] ==================== END OF MIGRATION ====================");
        CookClans.log("[SimpleClans] ==================== END OF MIGRATION ====================");
        CookClans.log("[SimpleClans] ==================== END OF MIGRATION ====================");
        ...
        CookClans.getInstance().setUUID(true);
    }
}
