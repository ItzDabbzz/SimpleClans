...
import me.itzdabbzz.cookclans.events.RequestEvent;
import me.itzdabbzz.cookclans.events.RequestFinishedEvent;
...
public final class RequestManager {
    ...
    public void processResults(Request req) {
        if (req.getType().equals(ClanRequest.START_WAR)) {
            ...
            if (req.getType().equals(ClanRequest.DEMOTE)) {
                ...
                if ( CookClans.getInstance().hasUUID() && demotedUniqueId != null) {
                    return;
                }
                ...
                if ( CookClans.getInstance().hasUUID() && promotedUniqueId == null) {
                    return;
                }
                if (denies.isEmpty()) {
        ...
        CookClans.getInstance().getServer().getPluginManager().callEvent(new RequestFinishedEvent(req));
    }
    ...
    public void ask(final Request req) {
        ...
        new BukkitRunnable() {
            @Override
            public void run() {
                CookClans.getInstance().getServer().getPluginManager().callEvent(new RequestEvent(req));
            }
        }.runTask(plugin);
    }
}
