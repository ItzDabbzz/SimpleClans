package me.itzdabbzz.cookclans.listeners;
...
import me.itzdabbzz.cookclans.executors.*;
...
public class SCPlayerListener implements Listener {
    ...
    public void onPlayerJoin(PlayerJoinEvent event) {
        ...
        if ( CookClans.getInstance().getSettingsManager().isBlacklistedWorld(player.getLocation().getWorld().getName())) {
            return;
        }

        ClanPlayer cp;
        if ( CookClans.getInstance().getSettingsManager().getUseBungeeCord()) {
            cp = CookClans.getInstance().getClanManager().getClanPlayerJoinEvent(player);
        } else {
            cp = CookClans.getInstance().getClanManager().getClanPlayer(player);
        }

        CookClans.getInstance().getStorageManager().updatePlayerNameAsync(player);
        CookClans.getInstance().getClanManager().updateLastSeen(player);
        ...
        if (event.getPlayer().isOp()) {
            for (String message : CookClans.getInstance().getMessages()) {
