...
import me.itzdabbzz.cookclans.events.HomeRegroupEvent;
import me.itzdabbzz.cookclans.events.PlayerHomeSetEvent;
...
public class HomeCommand {
    ...
    public void execute(Player player, String[] arg) {
        ...
        if (arg[0].equalsIgnoreCase("set")) {
            ...
            CookClans.getInstance().getServer().getPluginManager().callEvent(homeSetEvent);
        ...
        if (arg[0].equalsIgnoreCase("regroup")) {
            if (!CookClans.getInstance().getSettingsManager().getAllowReGroupCommand()) {
                ChatBlock.sendMessage(player, ChatColor.RED + plugin.getLang("insufficient.permissions"));
                return;
            }
            Location loc;
            ...
            CookClans.getInstance().getServer().getPluginManager().callEvent(homeRegroupEvent);
